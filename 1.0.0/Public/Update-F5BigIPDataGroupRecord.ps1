...
.EXAMPLE 
    Update-F5BigIPDataGroupRecord -F5BigIpAddress "XXXX.xxx.com" -Partition "common" -DataGroupName "DataGroupName" -DataGroupRecordName "DataGroupRecordName" -DataGroupRecordData "DataGroupRecordData" -F5Creds "CredObject"
...
#>
Function Update-F5BigIPDataGroupRecord {
    [CmdletBinding()]
    param(
    ...
    $Uri = "https://$($F5BigIpAddress)/mgmt/tm/ltm/data-group/internal/~$($Partition)~$($DataGroupName)?options=records+modify+{$($DataGroupRecordName){data+`"$($DataGroupRecordData)`"}}"
